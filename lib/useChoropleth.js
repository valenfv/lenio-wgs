...
export const useChoropleth = (svgRef, {
  ...
  React.useEffect(() => {
    ...
    const colorScale = d3.scaleQuantize()
      ...
      .range(['#00372f', '#0E564C', '#258F81', '#3DCDB9']);
    ...
    function render(_, topo) {
      const mouseOver = function (d) {
        ...
        const tooltipHtml = `
                <b>${countries[hoveredCountry.datum().id]}</b>
                <hr />
                <b>${indicators[selectedIndicator].indicator_name}</b>: ${worldData[hoveredCountry.datum().id]?.[selectedIndicator].value}
                <br />
                <b>POSITION</b>: ${worldData[hoveredCountry.datum().id]?.position}
            `;
